...
function ComboSingle() {
  ...
  const getMaxQuantity = () => {
    const quantityArray = combo?.listProduct.map((product) =>
      Math.floor(product.quantity / product.quantityProductInCombo)
    );
  ...
  const handleUpdateQuantity = (quantity) => {
    ...
    if (
      newQuantity + comboInCartQuantity > getMaxQuantity() ||
      newQuantity <= 1
    )
      return;
  ...
  const formattedDiscount = useCurrency(
    ((combo?.priceAfterDiscount * 100) / 90) * 0.1 || 0
  );
            ...
            <div className={styles.productSingleInfo}>
              ...
              {isOutOfStock && (
                ...
                >
                  ...
                  the maximum limit for this combo
                </p>
              )}
              <p>
                <button
                  ...
                  disabled={getMaxQuantity() > 0 ? false : true}
                >
